...
internal sealed class GetDoctorByIdQueryHandler(IDbConnectionFactory factory)
    : IQueryHandler<GetDoctorByIdQuery, DoctorDetailResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var doctor = await connection.QueryFirstOrDefaultAsync<DoctorRow>(doctorSql, new { request.DoctorId });
        if (doctor is null)
            return Error.NotFound("GetDoctorById.NotFound", $"Doctor with id {request.DoctorId} was not found.");
        ...
        var credentials = (await connection.QueryAsync<DoctorCredentialItem>(credentialsSql, new { request.DoctorId })).ToList();
        ...
        var specialties = (await connection.QueryAsync<DoctorSpecialtyItem>(specialtiesSql, new { request.DoctorId })).ToList();
