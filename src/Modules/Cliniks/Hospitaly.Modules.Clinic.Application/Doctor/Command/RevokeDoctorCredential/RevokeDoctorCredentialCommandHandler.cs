...
internal sealed class RevokeDoctorCredentialCommandHandler(
    IDoctorRepository doctorRepository,
    IUnitOfWork unitOfWork) : ICommandHandler<RevokeDoctorCredentialCommand, Success>
{
    public async Task<ErrorOr<Success>> Handle(
        RevokeDoctorCredentialCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var result = doctor.RevokeCredential(request.CredentialId);
        if (result.IsError)
